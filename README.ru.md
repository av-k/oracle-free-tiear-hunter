...
- **Публичный репо** (по умолчанию): неограниченные минуты — workflow запускается каждые 30 мин без изменений
- **Приватный репо**: 2 000 мин/месяц бесплатно. Измени cron в `hunter.yml` на `0 */2 * * *` (каждые 2 часа), чтобы уложиться в лимит

**Настройка количества попыток:**
Установи переменную репозитория `HUNT_ATTEMPTS` чтобы контролировать количество попыток за один запуск (по умолчанию: 15).
**Settings → Secrets and variables → Actions → Variables → New variable**.
Полезно для приватных репо, где нужно балансировать между частотой попыток и расходом минут.
